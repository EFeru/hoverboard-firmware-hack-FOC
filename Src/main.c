...
extern volatile uint32_t timeoutCntGen; // Timeout counter for the General
                                        // timeout (PPM, PWM, Nunchuk)
extern volatile uint8_t
    timeoutFlgGen; // Timeout Flag for the General timeout (PPM, PWM, Nunchuk)
extern uint8_t
    timeoutFlgADC; // Timeout Flag for for ADC Protection: 0 = OK, 1 = Problem
                   // detected (line disconnected or wrong ADC data)
extern uint8_t
...
static RemoteUartBusTelemetry rubTelemetry;
#endif
...
static int16_t
    steerRateFixdt; // local fixed-point variable for steering rate limiter
static int16_t
    speedRateFixdt; // local fixed-point variable for speed rate limiter
static int32_t
    steerFixdt; // local fixed-point variable for steering low-pass filter
static int32_t
...
static MultipleTap
    MultipleTapBrake; // define multiple tap functionality for the Brake pedal
...
int main(void) {
...
#ifdef MULTI_MODE_DRIVE
  if (adc_buffer.l_tx2 > input1[0].min + 50 &&
      adc_buffer.l_rx2 > input2[0].min + 50) {
    drive_mode = 2;
    max_speed = MULTI_MODE_DRIVE_M3_MAX;
    rate = MULTI_MODE_DRIVE_M3_RATE;
    rtP_Left.n_max = rtP_Right.n_max = MULTI_MODE_M3_N_MOT_MAX << 4;
    rtP_Left.i_max = rtP_Right.i_max = (MULTI_MODE_M3_I_MOT_MAX * A2BIT_CONV)
                                       << 4;
    ...
    rtP_Left.i_max = rtP_Right.i_max = (MULTI_MODE_M2_I_MOT_MAX * A2BIT_CONV)
                                       << 4;
  } else {
    drive_mode = 0;
    ...
    rtP_Left.i_max = rtP_Right.i_max = (MULTI_MODE_M1_I_MOT_MAX * A2BIT_CONV)
                                       << 4;
  }
...
#ifdef MULTI_MODE_DRIVE
  // Wait until triggers are released. Exit if timeout elapses (to unblock if
  // the inputs are not calibrated)
  int iTimeout = 0;
  while ((adc_buffer.l_rx2 + adc_buffer.l_tx2) >=
             (input1[0].min + input2[0].min) &&
         iTimeout++ < 300) {
    HAL_Delay(10);
  }
#endif
  ...
  while (1) {
    if (buzzerTimer - buzzerTimer_prev >
        16 * DELAY_IN_MAIN_LOOP) { // 1 ms = 16 ticks buzzerTimer
...
// ####### VARIANT_HOVERCAR #######
#if defined(VARIANT_HOVERCAR) || defined(VARIANT_SKATEBOARD) ||                \
    defined(ELECTRIC_BRAKE_ENABLE)
      uint16_t speedBlend; // Calculate speed Blend, a number between [0, 1] in
                           // fixdt(0,16,15)
      ...
      standstillHold(); // Apply Standstill Hold functionality. Only available
                        // and makes sense for VOLTAGE or TORQUE Mode
#endif

#ifdef VARIANT_HOVERCAR
      if (inIdx == CONTROL_ADC) { // Only use use implementation below if pedals
                                  // are in use (ADC input)
        if (speedAvgAbs < 60) {   // Check if Hovercar is physically close to
                                // standstill to enable Double tap detection on
                                // Brake pedal for Reverse functionality
          multipleTapDet(input1[inIdx].cmd, HAL_GetTick(),
                         &MultipleTapBrake); // Brake pedal in this case is
                                             // "input1" variable
        }

        if (input1[inIdx].cmd >
            30) { // If Brake pedal (input1) is pressed, bring to 0 also the
                  // Throttle pedal (input2) to avoid "Double pedal" driving
          input2[inIdx].cmd = (int16_t)((input2[inIdx].cmd * speedBlend) >> 15);
          cruiseControl((uint8_t)rtP_Left
                            .b_cruiseCtrlEna); // Cruise control deactivated by
                                               // Brake pedal if it was active
        }
      }
#endif

#ifdef ELECTRIC_BRAKE_ENABLE
      electricBrake(speedBlend,
                    MultipleTapBrake
                        .b_multipleTap); // Apply Electric Brake. Only available
                                         // and makes sense for TORQUE Mode
#endif
      ...
      if (inIdx == CONTROL_ADC) { // Only use use implementation below if pedals
                                  // are in use (ADC input)
        if (speedAvg >
            0) { // Make sure the Brake pedal is opposite to the direction of
                 // motion AND it goes to 0 as we reach standstill (to avoid
                 // Reverse driving by Brake pedal)
          input1[inIdx].cmd =
              (int16_t)((-input1[inIdx].cmd * speedBlend) >> 15);
        } else {
      ...
      if (input2[inIdx].cmd <
          0) { // When Throttle is negative, it acts as brake. This condition is
               // to make sure it goes to 0 as we reach standstill (to avoid
               // Reverse driving)
        if (speedAvg >
            0) { // Make sure the braking is opposite to the direction of motion
          input2[inIdx].cmd = (int16_t)((input2[inIdx].cmd * speedBlend) >> 15);
        } else {
          input2[inIdx].cmd =
              (int16_t)((-input2[inIdx].cmd * speedBlend) >> 15);
        }
      }
#endif
...
// ####### VARIANT_HOVERCAR #######
      ...
      if (inIdx == CONTROL_ADC) { // Only use use implementation below if pedals
        ...
        if (!MultipleTapBrake.b_multipleTap) { // Check driving direction
          speed = steer + speed; // Forward driving: in this case steer = Brake,
                                 // speed = Throttle
        } else {
          speed = steer - speed; // Reverse driving: in this case steer = Brake,
                                 // speed = Throttle
        }
        steer = 0; // Do not apply steering to avoid side effects if
                   // STEER_COEFFICIENT is NOT 0
      }
#endif

#if defined(TANK_STEERING) && !defined(VARIANT_HOVERCAR) &&                    \
    !defined(VARIANT_SKATEBOARD)
      // Tank steering (no mixing)
      cmdL = steer;
      cmdR = speed;
#else
                                           // ####### MIXER #######
      mixerFcn(speed << 4, steer << 4, &cmdR,
               &cmdL); // This function implements the equations above
#endif
...
// ####### SET OUTPUTS (if the target change is less than +/- 100) #######
      ...
      if (nunchuk_connected == 0) {
        cmdL =
            cmdL * 0.8f +
            (CLAMP(distanceErr +
                       (steering * ((float)MAX(ABS(distanceErr), 50)) * ROT_P),
                   -850, 850) *
             -0.2f);
        cmdR =
            cmdR * 0.8f +
      ...
      if ((distance / 1345.0) - setDistance > 0.5 &&
          (lastDistance / 1345.0) - setDistance >
              0.5) { // Error, robot too far away!
        enable = 0;
        beepLong(5);
#ifdef SUPPORT_LCD
    ...
    if (transpotter_counter % 100 == 0) {
      if (LCDerrorFlag == 1 && enable == 0) {
        ...
        // LCD_SetLocation(&lcd, 11, 1); LCD_WriteFloat(&lcd,MAX(ABS(currentR),
        // ABS(currentL)),2);
      }
    }
#endif
...
// ####### SIDEBOARDS HANDLING #######
    ...
    board_temp_adcFilt =
        (int16_t)(board_temp_adcFixdt >> 16); // convert fixed-point to integer
    ...
    left_dc_curr =
        -(rtU_Left.i_DCLink * 100) / A2BIT_CONV; // Left DC Link Current * 100
    right_dc_curr =
        -(rtU_Right.i_DCLink * 100) / A2BIT_CONV; // Right DC Link Current * 100
...
#else
        printf(
            "in1:%i in2:%i cmdL:%i cmdR:%i BatADC:%i BatV:%i TempADC:%i "
            "Temp:%i \r\n",
...
// ####### FEEDBACK SERIAL OUT #######
    ...
    if (main_loop_counter % 2 == 0) { // Send data periodically every 10 ms
      ...
      if (__HAL_DMA_GET_COUNTER(huart2.hdmatx) == 0) {
        ...
        Feedback.checksum =
            (uint16_t)(Feedback.start ^ Feedback.cmd1 ^ Feedback.cmd2 ^
    ...
    if (TEMP_POWEROFF_ENABLE && board_temp_deg_c >= TEMP_POWEROFF &&
      ...
      poweroff();
               ...
               rtY_Right.z_errCode) { // 1 beep (low pitch): Motor error,
                                      // disable motors
      enable = 0;
      beepCount(1, 24, 1);
      ...
      beepCount(4, 24, 1);
               ...
               board_temp_deg_c >=
                   TEMP_WARNING) { // 5 beeps (low pitch): Mainboard temperature
                                   // warning
      beepCount(5, 24, 1);
      ...
      beepCount(0, 10, 30);
               ...
               (((cmdR < -50 || cmdL < -50) && speedAvg < 0) ||
                MultipleTapBrake.b_multipleTap)) { // 1 beep fast (high pitch):
...
#ifdef VARIANT_REMOTE_UART_BUS
void sendRemoteUartBusTelemetry(uint8_t slave_id) {
  int16_t current_speed = (slave_id == SLAVE_ID) ? (int16_t)rtY_Left.n_mot
                                                 : (int16_t)rtY_Right.n_mot;
  int16_t current_dc =
      (slave_id == SLAVE_ID) ? left_dc_curr : right_dc_curr; // 0.01A units

  rubTelemetry.start = 0xABCD; // 0xABCD Little-Endian [0xCD, 0xAB]
  rubTelemetry.slave_id = slave_id;
  rubTelemetry.speed = current_speed;
  rubTelemetry.voltage = batVoltageCalib;
  rubTelemetry.current = current_dc;
  rubTelemetry.odometry = 0; // Odometry not implemented

  rubTelemetry.crc =
      crc16_ccitt((uint8_t *)&rubTelemetry, sizeof(RemoteUartBusTelemetry) - 2);

  // Note: We might want to wait or just discard if busy,
  // but "immediately after" suggests we should try to send.
  // Bus topology requires us to be quick.
  HAL_UART_Transmit_DMA(&huart3, (uint8_t *)&rubTelemetry,
                        sizeof(RemoteUartBusTelemetry));
}
#endif
