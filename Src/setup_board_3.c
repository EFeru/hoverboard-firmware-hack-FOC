...
pc11 usart3 dma1 channel2/3


         ADC1                    ADC2
------------------------------------------------
  NC   ---     dcr     |  NC   ---     dcl
  PA4  ch4     rlA     |  PA3  ch3     rlB
  PC4  ch14    rrB     |  PC5  ch15    rrC
  PC1  ch11    batt    |  ---  ----    l_tx2
  ---  ch16    temp    |  ---  ----    l_rx2
...
void MX_ADC1_Init(void) {
  ...
  sConfig.Channel = ADC_CHANNEL_11;  // dcr - DC current right wheel -- NOT USED
  sConfig.Rank    = 1;
  HAL_ADC_ConfigChannel(&hadc1, &sConfig);

  // sConfig.SamplingTime = ADC_SAMPLETIME_1CYCLE_5;
  sConfig.SamplingTime = ADC_SAMPLETIME_7CYCLES_5;
  sConfig.Channel = ADC_CHANNEL_4;  // rlA - phase A current left wheel -- PA4
  sConfig.Rank    = 2;
  HAL_ADC_ConfigChannel(&hadc1, &sConfig);

  sConfig.Channel = ADC_CHANNEL_14;  // rrB - phase B current right wheel -- PC4
  sConfig.Rank    = 3;
  HAL_ADC_ConfigChannel(&hadc1, &sConfig);

  sConfig.Channel = ADC_CHANNEL_11;  // batt1 - battery voltage -- PC1
  sConfig.Rank    = 4;
  HAL_ADC_ConfigChannel(&hadc1, &sConfig);
...
void MX_ADC2_Init(void) {
  ...
  sConfig.SamplingTime = ADC_SAMPLETIME_1CYCLE_5;
  sConfig.Channel = ADC_CHANNEL_11;     // dcl - DC current right wheel -- NOT USED
  sConfig.Rank    = 1;
  HAL_ADC_ConfigChannel(&hadc2, &sConfig);

  // sConfig.SamplingTime = ADC_SAMPLETIME_1CYCLE_5;
  sConfig.SamplingTime = ADC_SAMPLETIME_7CYCLES_5;
  sConfig.Channel = ADC_CHANNEL_3;      // rlB - phase B current left wheel -- PA4
  sConfig.Rank    = 2;
  HAL_ADC_ConfigChannel(&hadc2, &sConfig);

  sConfig.Channel = ADC_CHANNEL_15;     // rrC - phase C current left wheel -- PA4
  sConfig.Rank    = 3;
  HAL_ADC_ConfigChannel(&hadc2, &sConfig);

  sConfig.Channel = ADC_CHANNEL_11;     // adc1 - yet -- NOT USED
  sConfig.Rank    = 4;
  HAL_ADC_ConfigChannel(&hadc2, &sConfig);

  // sConfig.SamplingTime = ADC_SAMPLETIME_239CYCLES_5;   // Commented-out to make `uart-l-rx` ADC sample time the same as `uart-l-tx`
  sConfig.Channel = ADC_CHANNEL_11;     // adc2 - yet -- NOT USED
  sConfig.Rank    = 5;
  HAL_ADC_ConfigChannel(&hadc2, &sConfig);
