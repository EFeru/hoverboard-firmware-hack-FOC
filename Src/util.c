...
extern uint8_t buzzerCount; // global variable for the buzzer counts. can be 1,
                            // 2, 3, 4, 5, 6, 7...
...
extern volatile uint32_t
    timeoutCntGen; // global counter for general timeout counter
extern volatile uint8_t
    timeoutFlgGen; // global flag for general timeout counter
...
uint8_t timeoutFlgADC =
    0; // Timeout Flag for ADC Protection:    0 = OK, 1 = Problem detected (line
       // disconnected or wrong ADC data)
uint8_t timeoutFlgSerial =
    0; // Timeout Flag for Rx Serial command: 0 = OK, 1 = Problem detected (line
       // disconnected or wrong Rx data)
...
uint16_t VirtAddVarTab[NB_OF_VAR] = {
    1337}; // Virtual address defined by the user: 0xFFFF value is prohibited
static uint16_t saveValue = 0;
static uint8_t saveValue_valid = 0;
#elif !defined(VARIANT_HOVERBOARD) && !defined(VARIANT_TRANSPOTTER)
uint16_t VirtAddVarTab[NB_OF_VAR] = {1000, 1001, 1002, 1003, 1004, 1005, 1006,
                                     1007, 1008, 1009, 1010, 1011, 1012, 1013,
                                     1014, 1015, 1016, 1017, 1018};
#else
uint16_t VirtAddVarTab[NB_OF_VAR] = {
    1000}; // Dummy virtual address to avoid warnings
#endif
...
static uint16_t timeoutCntADC =
    ADC_PROTECT_TIMEOUT; // Timeout counter for ADC Protection
#endif
...
static uint16_t timeoutCntSerial_L =
    SERIAL_TIMEOUT; // Timeout counter for Rx Serial command
static uint8_t timeoutFlgSerial_L =
    0; // Timeout Flag for Rx Serial command: 0 = OK, 1 = Problem detected (line
       // disconnected or wrong Rx data)
#endif
...
static uint16_t timeoutCntSerial_R =
    SERIAL_TIMEOUT; // Timeout counter for Rx Serial command
static uint8_t timeoutFlgSerial_R =
    0; // Timeout Flag for Rx Serial command: 0 = OK, 1 = Problem detected (line
       // disconnected or wrong Rx data)
#endif
...
// static RemoteUartBusCommand rubCommand;
// static RemoteUartBusConfig rubConfig;
static int16_t rubSpeedL;
static int16_t rubSpeedR;
static uint8_t rubStateL;
static uint8_t rubStateR;
#endif

#if defined(SUPPORT_BUTTONS) || defined(SUPPORT_BUTTONS_LEFT) ||               \
    defined(SUPPORT_BUTTONS_RIGHT)
...
void BLDC_Init(void) {
  ...
  rtP_Left.b_angleMeasEna = 0; // Motor angle input: 0 = estimated angle, 1 =
                               // measured angle (e.g. if encoder is available)
  rtP_Left.z_selPhaCurMeasABC = 0; // Left motor measured current phases {Green,
                                   // Blue} = {iA, iB} -> do NOT change
  rtP_Left.z_ctrlTypSel = CTRL_TYP_SEL;
  rtP_Left.b_diagEna = DIAG_ENA;
  rtP_Left.i_max = (I_MOT_MAX * A2BIT_CONV) << 4; // fixdt(1,16,4)
  rtP_Left.n_max = N_MOT_MAX << 4;                // fixdt(1,16,4)
  rtP_Left.b_fieldWeakEna = FIELD_WEAK_ENA;
  rtP_Left.id_fieldWeakMax = (FIELD_WEAK_MAX * A2BIT_CONV)
                             << 4;             // fixdt(1,16,4)
  ...
  rtP_Right =
      rtP_Left; // Copy the Left motor parameters to the Right motor parameters
  rtP_Right.z_selPhaCurMeasABC =
      1; // Right motor measured current phases {Blue, Yellow} = {iB, iC} -> do
         // NOT change
...
void Input_Init(void) {
...
#endif
#if defined(DEBUG_SERIAL_USART3) || defined(CONTROL_SERIAL_USART3) ||          \
    defined(FEEDBACK_SERIAL_USART3) || defined(SIDEBOARD_SERIAL_USART3)
  UART3_Init();
#endif
#if defined(DEBUG_SERIAL_USART2) || defined(CONTROL_SERIAL_USART2) ||          \
    defined(SIDEBOARD_SERIAL_USART2)
  ...
  if (writeCheck == FLASH_WRITE_KEY) {
    ...
    for (uint8_t i = 0; i < INPUTS_NR; i++) {
      if (input1[i].typDef ==
          3) { // If Input type defined is 3 (auto), identify the input type
               // based on the values from config.h
        input1[i].typ =
            checkInputType(input1[i].min, input1[i].mid, input1[i].max);
      } else {
        input1[i].typ = input1[i].typDef;
      }
      if (input2[i].typDef == 3) {
        input2[i].typ =
            checkInputType(input2[i].min, input2[i].mid, input2[i].max);
      } else {
...
/**
 * @brief  Disable Rx Errors detection interrupts on UART peripheral (since we
 * do not want DMA to be stopped) The incorrect data will be filtered based on
 * the START_FRAME and checksum.
 * @param  huart: UART handle.
 * @retval None
 */
#if defined(DEBUG_SERIAL_USART2) || defined(CONTROL_SERIAL_USART2) ||          \
    defined(SIDEBOARD_SERIAL_USART2) || defined(DEBUG_SERIAL_USART3) ||        \
    defined(CONTROL_SERIAL_USART3) || defined(SIDEBOARD_SERIAL_USART3)
void UART_DisableRxErrors(UART_HandleTypeDef *huart) {
  CLEAR_BIT(huart->Instance->CR1,
            USART_CR1_PEIE); /* Disable PE (Parity Error) interrupts */
  CLEAR_BIT(huart->Instance->CR3,
            USART_CR3_EIE); /* Disable EIE (Frame error, noise error, overrun
                               error) interrupts */
}
#endif
...
void calcAvgSpeed(void) {
  // Calculate measured average speed. The minus sign (-) is because motors spin
...
// Average only if both motors are enabled
  ...
  // Handle the case when SPEED_COEFFICIENT sign is negative (which is when most
...
void adcCalibLim(void) {
  ...
  while (!HAL_GPIO_ReadPin(BUTTON_PORT, BUTTON_PIN) &&
         input_cal_timeout++ < 4000) { // 20 sec timeout
    readInputRaw();
    ...
    INPUT1_MID_temp =
        (int16_t)(input1_fixdt >>
                  16); // CLAMP(input1_fixdt >> 16, INPUT1_MIN, INPUT1_MAX); //
                       // convert fixed-point to integer
    INPUT2_MID_temp =
        (int16_t)(input2_fixdt >>
                  16); // CLAMP(input2_fixdt >> 16, INPUT2_MIN, INPUT2_MAX);
  ...
  printf("Input1 is ");
#endif
  uint8_t input1TypTemp =
      checkInputType(INPUT1_MIN_temp, INPUT1_MID_temp, INPUT1_MAX_temp);
  if (input1TypTemp == input1[inIdx].typDef ||
      input1[inIdx].typDef == 3) { // Accept calibration only if the type is
                                   // correct OR type was set to 3 (auto)
#if defined(DEBUG_SERIAL_USART2) || defined(DEBUG_SERIAL_USART3)
    printf("..OK\r\n");
#endif
  } else {
  ...
  printf("Input2 is ");
#endif
  uint8_t input2TypTemp =
      checkInputType(INPUT2_MIN_temp, INPUT2_MID_temp, INPUT2_MAX_temp);
  if (input2TypTemp == input2[inIdx].typDef ||
      input2[inIdx].typDef == 3) { // Accept calibration only if the type is
                                   // correct OR type was set to 3 (auto)
#if defined(DEBUG_SERIAL_USART2) || defined(DEBUG_SERIAL_USART3)
    printf("..OK\r\n");
#endif
  ...
  if (input1TypTemp != 0 || input2TypTemp != 0) {
    ...
    printf("Limits Input1: TYP:%i MIN:%i MID:%i MAX:%i\r\nLimits Input2: "
           "TYP:%i MIN:%i MID:%i MAX:%i\r\n",
           input1[inIdx].typ, input1[inIdx].min, input1[inIdx].mid,
           input1[inIdx].max, input2[inIdx].typ, input2[inIdx].min,
           input2[inIdx].mid, input2[inIdx].max);
#endif
  } else {
#if defined(DEBUG_SERIAL_USART2) || defined(DEBUG_SERIAL_USART3)
...
/*
 * Update Maximum Motor Current Limit (via ADC1) and Maximum Speed Limit (via
 * ADC2) Procedure:
 * - press the power button for more than 5 sec and immediatelly after the beep
 * sound press one more time shortly
...
void updateCurSpdLim(void) {
  ...
  while (!HAL_GPIO_ReadPin(BUTTON_PORT, BUTTON_PIN) &&
         cur_spd_timeout++ < 2000) { // 10 sec timeout
    readInputRaw();
    filtLowPass32(input1[inIdx].raw, FILTER, &input1_fixdt);
    filtLowPass32(input2[inIdx].raw, FILTER, &input2_fixdt);
    HAL_Delay(5);
  ...
  cur_factor = CLAMP((input1_fixdt - (input1[inIdx].min << 16)) /
                         (input1[inIdx].max - input1[inIdx].min),
                     6553, 65535); // ADC1, MIN_cur(10%) = 1.5 A
  spd_factor = CLAMP((input2_fixdt - (input2[inIdx].min << 16)) /
                         (input2[inIdx].max - input2[inIdx].min),
                     3276, 65535); // ADC2, MIN_spd(5%)  = 50 rpm

  if (input1[inIdx].typ != 0) {
    // Update current limit
    rtP_Left.i_max = rtP_Right.i_max =
        (int16_t)((I_MOT_MAX * A2BIT_CONV * cur_factor) >>
                  12); // fixdt(0,16,16) to fixdt(1,16,4)
    cur_spd_valid = 1; // Mark update to be saved in Flash at shutdown
  }

  if (input2[inIdx].typ != 0) {
    // Update speed limit
    rtP_Left.n_max = rtP_Right.n_max =
        (int16_t)((N_MOT_MAX * spd_factor) >>
                  12);  // fixdt(0,16,16) to fixdt(1,16,4)
    cur_spd_valid += 2; // Mark update to be saved in Flash at shutdown
  }

#if defined(DEBUG_SERIAL_USART2) || defined(DEBUG_SERIAL_USART3)
  // cur_spd_valid: 0 = No limit changed, 1 = Current limit changed, 2 = Speed
  // limit changed, 3 = Both limits changed
  printf("Limits (%i)\r\nCurrent: fixdt:%li factor%i i_max:%i \r\nSpeed: "
         "fixdt:%li factor:%i n_max:%i\r\n",
         cur_spd_valid, input1_fixdt, cur_factor, rtP_Left.i_max, input2_fixdt,
         spd_factor, rtP_Left.n_max);
#endif
 ...
 * This function uses Cruise Control to provide an anti-roll functionality at
 * standstill. Only available and makes sense for FOC VOLTAGE or FOC TORQUE
 * mode.
 *
 * Input:  none
...
void standstillHold(void) {
    ...
    (CTRL_MOD_REQ != SPD_MODE)
  if (!rtP_Left
           .b_cruiseCtrlEna) { // If Stanstill in NOT Active -> try Activation
    if (((input1[inIdx].cmd > 50 || input2[inIdx].cmd < -50) &&
         speedAvgAbs <
             30) // Check if Brake is pressed AND measured speed is small
        || (input2[inIdx].cmd < 20 &&
            speedAvgAbs <
                5)) { // OR Throttle is small AND measured speed is very small
    ...
    if (input1[inIdx].cmd < 20 && input2[inIdx].cmd > 50 &&
        !cruiseCtrlAcv) { // Check if Brake is released AND Throttle is pressed
 ...
 * In case of TORQUE mode, this function replaces the motor "freewheel" with a
 * constant braking when the input torque request is 0. This is useful when a
 * small amount of motor braking is desired instead of "freewheel".
 *
...
void electricBrake(uint16_t speedBlend, uint8_t reverseDir) {
    ...
    (CTRL_MOD_REQ == TRQ_MODE)
  int16_t brakeVal;

  // Make sure the Brake pedal is opposite to the direction of motion AND it
  // goes to 0 as we reach standstill (to avoid Reverse driving)
  if (speedAvg > 0) {
  ...
  if (input2[inIdx].cmd >= 0 && input2[inIdx].cmd < ELECTRIC_BRAKE_THRES) {
    input2[inIdx].cmd =
        MAX(brakeVal, ((ELECTRIC_BRAKE_THRES - input2[inIdx].cmd) * brakeVal) /
                          ELECTRIC_BRAKE_THRES);
  } else if (input2[inIdx].cmd >= -ELECTRIC_BRAKE_THRES &&
             input2[inIdx].cmd < 0) {
    input2[inIdx].cmd =
        MIN(brakeVal, ((ELECTRIC_BRAKE_THRES + input2[inIdx].cmd) * brakeVal) /
                          ELECTRIC_BRAKE_THRES);
  } else if (input2[inIdx].cmd >= ELECTRIC_BRAKE_THRES) {
    input2[inIdx].cmd =
        MAX(brakeVal, ((input2[inIdx].cmd - ELECTRIC_BRAKE_THRES) * INPUT_MAX) /
                          (INPUT_MAX - ELECTRIC_BRAKE_THRES));
  } else { // when (input2.cmd < -ELECTRIC_BRAKE_THRES)
    input2[inIdx].cmd =
        MIN(brakeVal, ((input2[inIdx].cmd + ELECTRIC_BRAKE_THRES) * INPUT_MIN) /
...
void cruiseControl(uint8_t button) {
  ...
  if (button && !rtP_Left.b_cruiseCtrlEna) { // Cruise control activated
    ...
    beepShortMany(2, 1); // 200 ms beep delay. Acts as a debounce also.
             ...
             !standstillAcv) { // Cruise control deactivated if no Standstill
                               // Hold is active
    rtP_Left.b_cruiseCtrlEna = 0;
    rtP_Right.b_cruiseCtrlEna = 0;
    cruiseCtrlAcv = 0;
...
/*
 * Check Input Type
 * This function identifies the input type: 0: Disabled, 1: Normal Pot, 2:
 * Middle Resting Pot
 */
int checkInputType(int16_t min, int16_t mid, int16_t max) {
  ...
  if ((min / threshold) == (max / threshold) ||
      (mid / threshold) == (max / threshold) || min > max || mid > max) {
    type = 0;
#if defined(DEBUG_SERIAL_USART2) || defined(DEBUG_SERIAL_USART3)
    printf(
        "ignored"); // (MIN and MAX) OR (MID and MAX) are close, disable input
#endif
  } else {
...
#ifdef CONTROL_ADC
    if ((min + ADC_MARGIN - ADC_PROTECT_THRESH) > 0 &&
        (max - ADC_MARGIN + ADC_PROTECT_THRESH) < 4095) {
#if defined(DEBUG_SERIAL_USART2) || defined(DEBUG_SERIAL_USART3)
 ...
 * This function realizes dead-band around 0 and scales the input between
 * [out_min, out_max]
 */
void calcInputCmd(InputStruct *in, int16_t out_min, int16_t out_max) {
  ...
  case 2: // Input is a mid resting pot
    if (in->raw > in->mid - in->dband && in->raw < in->mid + in->dband) {
      ...
      in->cmd = CLAMP(MAP(in->raw, in->mid + in->dband, in->max, 0, out_max), 0,
                      out_max);
    } else {
      in->cmd = CLAMP(MAP(in->raw, in->mid - in->dband, in->min, 0, out_min),
                      out_min, 0);
    }
    break;
...
void readInputRaw(void) {
  ...
  if (inIdx == CONTROL_SERIAL_USART2) {
    ...
    for (uint8_t i = 0; i < (IBUS_NUM_CHANNELS * 2); i += 2) {
      ibusL_captured_value[(i / 2)] =
          CLAMP(commandL.channels[i] + (commandL.channels[i + 1] << 8) - 1000,
                0, INPUT_MAX); // 1000-2000 -> 0-1000
    }
...
#ifdef CONTROL_IBUS
    for (uint8_t i = 0; i < (IBUS_NUM_CHANNELS * 2); i += 2) {
      ibusR_captured_value[(i / 2)] =
          CLAMP(commandR.channels[i] + (commandR.channels[i + 1] << 8) - 1000,
                0, INPUT_MAX); // 1000-2000 -> 0-1000
    }
...
void handleTimeout(void) {
#ifdef CONTROL_ADC
  if (inIdx == CONTROL_ADC) {
    // If input1 or Input2 is either below MIN - Threshold or above MAX +
    // Threshold, ADC protection timeout
  ...
  if (timeoutCntSerial_L++ >= SERIAL_TIMEOUT) { // Timeout qualification
    ...
    ((defined(CONTROL_SERIAL_USART2) && CONTROL_SERIAL_USART2 == 1) ||         \
    ...
    if (Sideboard_L.sensors &
        SWA_SET) { // If SWA is set, switch to Sideboard control
      inIdx = SIDEBOARD_SERIAL_USART2;
    } else {
      inIdx = !SIDEBOARD_SERIAL_USART2;
    }
#elif defined(DUAL_INPUTS) &&                                                  \
    (defined(CONTROL_SERIAL_USART2) && CONTROL_SERIAL_USART2 == 1)
    inIdx =
        1; // Switch to Auxiliary input in case of NO Timeout on Auxiliary input
#endif
  }
#if (defined(CONTROL_SERIAL_USART2) && CONTROL_SERIAL_USART2 == 0) ||          \
    (defined(SIDEBOARD_SERIAL_USART2) && SIDEBOARD_SERIAL_USART2 == 0 &&       \
     !defined(VARIANT_HOVERBOARD))
  timeoutFlgSerial =
      timeoutFlgSerial_L; // Report Timeout only on the Primary Input
#endif
#endif
  ...
  if (timeoutCntSerial_R++ >= SERIAL_TIMEOUT) { // Timeout qualification
    ...
    (defined(SIDEBOARD_SERIAL_USART3) && SIDEBOARD_SERIAL_USART3 == 0 &&       \
     !defined(VARIANT_HOVERBOARD))
  timeoutFlgSerial =
      timeoutFlgSerial_R; // Report Timeout only on the Primary Input
#endif
#endif
...
#if defined(CONTROL_NUNCHUK) || defined(SUPPORT_NUNCHUK) ||                    \
    defined(VARIANT_TRANSPOTTER) || defined(CONTROL_PPM_LEFT) ||               \
    defined(CONTROL_PPM_RIGHT) || defined(CONTROL_PWM_LEFT) ||                 \
    defined(CONTROL_PWM_RIGHT)
  if (timeoutCntGen++ >= TIMEOUT) { // Timeout qualification
#if defined(CONTROL_NUNCHUK) || defined(SUPPORT_NUNCHUK) ||                    \
    defined(VARIANT_TRANSPOTTER) ||                                            \
    (defined(CONTROL_PPM_LEFT) && CONTROL_PPM_LEFT == 0) ||                    \
    (defined(CONTROL_PPM_RIGHT) && CONTROL_PPM_RIGHT == 0) ||                  \
    (defined(CONTROL_PWM_LEFT) && CONTROL_PWM_LEFT == 0) ||                    \
    (defined(CONTROL_PWM_RIGHT) && CONTROL_PWM_RIGHT == 0)
    timeoutFlgGen = 1; // Report Timeout only on the Primary Input
    timeoutCntGen = TIMEOUT;
#endif
#if defined(DUAL_INPUTS) &&                                                    \
    ((defined(CONTROL_PPM_LEFT) && CONTROL_PPM_LEFT == 1) ||                   \
     (defined(CONTROL_PPM_RIGHT) && CONTROL_PPM_RIGHT == 1) ||                 \
     (defined(CONTROL_PWM_LEFT) && CONTROL_PWM_LEFT == 1) ||                   \
     (defined(CONTROL_PWM_RIGHT) && CONTROL_PWM_RIGHT == 1))
    inIdx = 0; // Switch to Primary input in case of Timeout on Auxiliary input
#endif
  } else {
#if defined(DUAL_INPUTS) &&                                                    \
  ...
  if (timeoutFlgADC || timeoutFlgSerial || timeoutFlgGen) {
    ctrlModReq = OPEN_MODE; // Request OPEN_MODE. This will bring the motor
                            // power to 0 in a controlled way
    input1[inIdx].cmd = 0;
    input2[inIdx].cmd = 0;
  } else {
...
#if defined(CRUISE_CONTROL_SUPPORT) &&                                         \
    (defined(SUPPORT_BUTTONS) || defined(SUPPORT_BUTTONS_LEFT) ||              \
     defined(SUPPORT_BUTTONS_RIGHT))
 ...
 * - this function is called for every USART IDLE line detection, in the USART
 * interrupt handler
 */
void usart2_rx_check(void) {
  ...
  pos = rx_buffer_L_len -
        __HAL_DMA_GET_COUNTER(
            huart2.hdmarx); // Calculate current position in buffer
#endif

#if defined(DEBUG_SERIAL_USART2)
  if (pos != old_pos) {  // Check change in received data
    if (pos > old_pos) { // "Linear" buffer mode: check if current position is
                         // over previous one
      usart_process_debug(&rx_buffer_L[old_pos], pos - old_pos); // Process data
    } else { // "Overflow" buffer mode
      usart_process_debug(&rx_buffer_L[old_pos],
                          rx_buffer_L_len - old_pos); // Process data
  ...
  if (pos != old_pos) { // Check change in received data
    ptr =
        (uint8_t
             *)&commandL_raw; // Initialize the pointer with command_raw address
    if (pos > old_pos &&
        (pos - old_pos) ==
            commandL_len) { // "Linear" buffer mode: check if current position
                            // is over previous one AND data length equals
                            // expected length
      memcpy(ptr, &rx_buffer_L[old_pos],
             commandL_len); // Copy data. This is possible only if command_raw
                            // is contiguous! (meaning all the structure members
                            // have the same size)
      usart_process_command(&commandL_raw, &commandL, 2); // Process data
    } else if ((rx_buffer_L_len - old_pos + pos) ==
               commandL_len) { // "Overflow" buffer mode: check if data length
                               // equals expected length
      memcpy(ptr, &rx_buffer_L[old_pos],
             rx_buffer_L_len -
                 old_pos); // First copy data from the end of buffer
      if (pos > 0) {       // Check and continue with beginning of buffer
        ptr += rx_buffer_L_len -
               old_pos; // Move to correct position in command_raw
  ...
  uint8_t *ptr;
  if (pos != old_pos) {                // Check change in received data
    ptr = (uint8_t *)&Sideboard_L_raw; // Initialize the pointer with
                                       // Sideboard_raw address
    if (pos > old_pos &&
        (pos - old_pos) ==
            Sideboard_L_len) { // "Linear" buffer mode: check if current
                               // position is over previous one AND data length
                               // equals expected length
      memcpy(ptr, &rx_buffer_L[old_pos],
             Sideboard_L_len); // Copy data. This is possible only if
                               // Sideboard_raw is contiguous! (meaning all the
                               // structure members have the same size)
      usart_process_sideboard(&Sideboard_L_raw, &Sideboard_L,
                              2); // Process data
    } else if ((rx_buffer_L_len - old_pos + pos) ==
               Sideboard_L_len) { // "Overflow" buffer mode: check if data
                                  // length equals expected length
      memcpy(ptr, &rx_buffer_L[old_pos],
             rx_buffer_L_len -
                 old_pos); // First copy data from the end of buffer
      if (pos > 0) {       // Check and continue with beginning of buffer
        ptr += rx_buffer_L_len -
               old_pos; // Move to correct position in Sideboard_raw
        memcpy(ptr, &rx_buffer_L[0], pos); // Copy remaining data
      }
      usart_process_sideboard(&Sideboard_L_raw, &Sideboard_L,
                              2); // Process data
  ...
  if (old_pos == rx_buffer_L_len) { // Check and manually update if we reached
                                    // end of buffer
    old_pos = 0;
  }
#endif
...
void usart3_rx_check(void) {
  ...
  pos = rx_buffer_R_len -
        __HAL_DMA_GET_COUNTER(
            huart3.hdmarx); // Calculate current position in buffer
#endif
  ...
  if (pos != old_pos) {  // Check change in received data
    if (pos > old_pos) { // "Linear" buffer mode: check if current position is
      ...
      usart_process_debug(&rx_buffer_R[old_pos],
                          rx_buffer_R_len - old_pos);  // Process data
  ...
  if (pos != old_pos) { // Check change in received data
    ptr =
        (uint8_t
             *)&commandR_raw; // Initialize the pointer with command_raw address
    if (pos > old_pos &&
        (pos - old_pos) ==
            commandR_len) { // "Linear" buffer mode: check if current position
                            // is over previous one AND data length equals
                            // expected length
      memcpy(ptr, &rx_buffer_R[old_pos],
             commandR_len); // Copy data. This is possible only if command_raw
                            // is contiguous! (meaning all the structure members
                            // have the same size)
      usart_process_command(&commandR_raw, &commandR, 3); // Process data
    } else if ((rx_buffer_R_len - old_pos + pos) ==
               commandR_len) { // "Overflow" buffer mode: check if data length
                               // equals expected length
      memcpy(ptr, &rx_buffer_R[old_pos],
             rx_buffer_R_len -
                 old_pos); // First copy data from the end of buffer
  ...
  uint8_t *ptr;
  if (pos != old_pos) {                // Check change in received data
    ptr = (uint8_t *)&Sideboard_R_raw; // Initialize the pointer with
                                       // Sideboard_raw address
    if (pos > old_pos &&
        (pos - old_pos) ==
            Sideboard_R_len) { // "Linear" buffer mode: check if current
                               // position is over previous one AND data length
                               // equals expected length
      memcpy(ptr, &rx_buffer_R[old_pos],
             Sideboard_R_len); // Copy data. This is possible only if
                               // Sideboard_raw is contiguous! (meaning all the
                               // structure members have the same size)
      usart_process_sideboard(&Sideboard_R_raw, &Sideboard_R,
                              3); // Process data
    } else if ((rx_buffer_R_len - old_pos + pos) ==
               Sideboard_R_len) { // "Overflow" buffer mode: check if data
                                  // length equals expected length
      memcpy(ptr, &rx_buffer_R[old_pos],
             rx_buffer_R_len -
                 old_pos); // First copy data from the end of buffer
  ...
  if (pos != old_pos) {
    ...
    if (pos > old_pos) {
      bytes_to_process = pos - old_pos;
      usart_process_remote_uart_bus(&rx_buffer_R[old_pos], bytes_to_process);
    } else {
      bytes_to_process = rx_buffer_R_len - old_pos;
      usart_process_remote_uart_bus(&rx_buffer_R[old_pos], bytes_to_process);
      if (pos > 0) {
        usart_process_remote_uart_bus(&rx_buffer_R[0], pos);
      }
    }
  }
#endif
...
#if defined(DEBUG_SERIAL_USART3) || defined(CONTROL_SERIAL_USART3) ||          \
    defined(SIDEBOARD_SERIAL_USART3) || defined(VARIANT_REMOTE_UART_BUS)
  old_pos = pos;                    // Update old position
  if (old_pos == rx_buffer_R_len) { // Check and manually update if we reached
                                    // end of buffer
    old_pos = 0;
  }
#endif
 ...
 * - if the command_in data is valid (correct START_FRAME and checksum) copy the
 * command_in to command_out
 */
...
void usart_process_command(SerialCommand *command_in,
  ...
  if (command_in->start == IBUS_LENGTH && command_in->type == IBUS_COMMAND) {
    ...
    if (ibus_chksum ==
        (uint16_t)((command_in->checksumh << 8) + command_in->checksuml)) {
  ...
  if (command_in->start == SERIAL_START_FRAME) {
    checksum =
        (uint16_t)(command_in->start ^ command_in->steer ^ command_in->speed);
 ...
 * - if the Sideboard_in data is valid (correct START_FRAME and checksum) copy
 * the Sideboard_in to Sideboard_out
 */
#if defined(SIDEBOARD_SERIAL_USART2) || defined(SIDEBOARD_SERIAL_USART3)
void usart_process_sideboard(SerialSideboard *Sideboard_in,
  ...
  if (Sideboard_in->start == SERIAL_START_FRAME) {
    checksum = (uint16_t)(Sideboard_in->start ^ Sideboard_in->pitch ^
                          Sideboard_in->dPitch ^ Sideboard_in->cmd1 ^
...
// brakePressed == 0, turn off led
  ...
  if (main_loop_counter % BAT_BLINK_INTERVAL ==
      0) { //  | RED (LED1) | YELLOW (LED3) | GREEN (LED2) |
...
void sideboardSensors(uint8_t sensors) {
    ...
    (defined(SIDEBOARD_SERIAL_USART2) || defined(SIDEBOARD_SERIAL_USART3))
  ...
  if (inIdx == sideboardIdx) { // Use Sideboard data
    sensor1_index =
        2 + ((sideboardSns & SWB_SET) >>
             9); // SWB on RC transmitter is used to change Control Type
    if (sensor1_index == 2) { // FOC control Type
      sensor1_index =
          (sideboardSns & SWC_SET) >>
          11; // SWC on RC transmitter is used to change Control Mode
    }
    sensor1_trig = sensor1_index !=
                   sensor1_prev; // rising or falling edge change detection
    ...
    sensor1_trig =
        (sensors & SENSOR1_SET) && !sensor1_prev; // rising edge detection
  ...
  static uint8_t sensor2_index =
      1; // holds the press index number for sensor2, when used as a button

  // Override in case the Sideboard control is Active
  if (inIdx == sideboardIdx) { // Use Sideboard data
    sensor2_index =
        (sideboardSns & SWD_SET) >> 13; // SWD on RC transmitter is used to
                                        // Activate/Deactivate Field Weakening
    sensor2_trig = sensor2_index !=
                   sensor2_prev; // rising or falling edge change detection
    if (inIdx != inIdx_prev) {   // Force one update at Input idx change
      sensor2_trig = 1;
    }
    ...
    sensor2_trig =
        (sensors & SENSOR2_SET) && !sensor2_prev; // rising edge detection
...
void poweroffPressCheck(void) {
  ...
  if (HAL_GPIO_ReadPin(BUTTON_PORT, BUTTON_PIN)) {
    ...
    if (cnt_press >= 5 * 100) { // Check if press is more than 5 sec
      ...
      if (HAL_GPIO_ReadPin(
              BUTTON_PORT,
              BUTTON_PIN)) { // Double press: Adjust Max Current, Max Speed
        while (HAL_GPIO_ReadPin(BUTTON_PORT, BUTTON_PIN)) {
          HAL_Delay(10);
        }
        beepLong(8);
 ...
 *
 * Example:
 * If coef = 0.8 (in floating point), then coef = 0.8 * 2^16 = 52429 (in
 * fixed-point) filtLowPass16(u, 52429, &y); yint = (int16_t)(y >> 16); // the
 * integer output is the fixed-point ouput shifted by 16 bits
 */
void filtLowPass32(int32_t u, uint16_t coef, int32_t *y) {
  int64_t tmp;
  tmp = ((int64_t)((u << 4) - (*y >> 12)) * coef) >> 4;
  tmp = CLAMP(tmp, -2147483648LL,
              2147483647LL); // Overflow protection: 2147483647LL = 2^31 - 1
  *y = (int32_t)tmp + (*y);
}
// Old filter
// Inputs:       u     = int16
...
// Parameters:   coef  = fixdt(0,16,16) = [0,65535U]
// yint = (int16_t)(y >> 20); // the integer output is the fixed-point ouput
// shifted by 20 bits void filtLowPass32(int16_t u, uint16_t coef, int32_t *y) {
//   int32_t tmp;
 ...
 * Parameters:   rate  = fixdt(1,16,4) = [0, 32767] Do NOT make rate negative
 * (>32767)
 */
 ...
 * This function detects multiple tap presses, such as double tapping, triple
 * tapping, etc. Inputs:       u = int16_t (input signal); timeNow = uint32_t
 * (current time) Outputs:      x->b_multipleTap (get the output here)
 */
...
#ifdef VARIANT_REMOTE_UART_BUS
void usart_process_remote_uart_bus(uint8_t *data, uint32_t len) {
  static uint8_t msg_buf[255];
  ...
  for (uint32_t i = 0; i < len; i++) {
    ...
    if (msg_pos == 2) { // Type byte
      switch (msg_buf[1]) {
      case 0:
        msg_len = sizeof(RemoteUartBusCommand);
        break;
      case 2:
        msg_len = sizeof(RemoteUartBusConfig);
        break;
      default:
        msg_pos = 0; // Unknown type
        continue;
      }
    }

    if (msg_len > 0 && msg_pos == msg_len) {
      // Entire message received
      uint16_t calculated_crc = crc16_ccitt(msg_buf, msg_len - 2);

      // CRC is stored Little-Endian [LSB, MSB]
      uint16_t received_crc =
          (uint16_t)((msg_buf[msg_len - 1] << 8) | msg_buf[msg_len - 2]);

      if (calculated_crc == received_crc) {
        uint8_t slave_id = msg_buf[2];
        if (slave_id == SLAVE_ID || slave_id == (uint8_t)(SLAVE_ID + 1)) {
          if (msg_buf[1] == 0x00) { // Type 0: Basic Motor Control
            RemoteUartBusCommand *cmd = (RemoteUartBusCommand *)msg_buf;
            ...
            sendRemoteUartBusTelemetry(slave_id);
          } else if (msg_buf[1] == 0x02) { // Type 2: Configuration
            // stay alive
            timeoutFlgSerial_R = 0;
            timeoutCntSerial_R = 0;
            sendRemoteUartBusTelemetry(slave_id);
