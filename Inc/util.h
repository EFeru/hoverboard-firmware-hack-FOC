...
// Remote UART Bus Structures
#ifdef VARIANT_REMOTE_UART_BUS
typedef struct {
  uint8_t start; // 0x2F '/'
  uint8_t type;  // 0x00 for Basic Motor Control
  uint8_t slave_id;
  int16_t speed; // Little-Endian
  uint8_t state;
  uint16_t crc; // Little-Endian
} __attribute__((packed)) RemoteUartBusCommand;

typedef struct {
  uint8_t start; // 0x2F '/'
  uint8_t type;  // 0x02 for Configuration
  uint8_t slave_id;
  float batt_full;  // IEEE 754 Little-Endian
  float batt_empty; // IEEE 754 Little-Endian
  uint8_t drive_mode;
  int8_t new_id;
  uint16_t crc; // Little-Endian
} __attribute__((packed)) RemoteUartBusConfig;

typedef struct {
  uint16_t start; // 0xABCD Little-Endian
  uint8_t slave_id;
  int16_t speed;    // Little-Endian
  uint16_t voltage; // 0.1V units, Little-Endian
  int16_t current;  // 0.1A units, Little-Endian
  int32_t odometry; // Little-Endian
  uint16_t crc;     // Little-Endian
} __attribute__((packed)) RemoteUartBusTelemetry;
#endif
...
void usart_process_sideboard(SerialSideboard *Sideboard_in,
                             SerialSideboard *Sideboard_out, uint8_t usart_idx);
#endif
